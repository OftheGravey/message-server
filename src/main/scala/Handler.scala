...
class Handler(var clientSocket: Socket) extends Thread {
  ...
  def acceptStateChange(
      expectedStates: Array[ClientState]
  ): StateChangeStatus = {
  ...
  def authenticate(): AuthReturn = {
    ...
    if (clientState == ClientState.Login) {
      val attemptLogin =
        AccessManager.autheticateUser(possibleUsername, password)
      attemptLogin match {
        case NoAccessError =>
  ...
  def authenticationLoop(): AuthReturn = {
    ...
    while (true) {
      ...
      if (
        authResult == AuthReturn.AuthSuccess || authResult == AuthReturn.AuthFailure
      ) {
        return authResult
      }
    }
  ...
  def receiveClientUserMessage() {
    ...
    val response = UserMessageResponse
      .newBuilder()
